...
class CRUD(object):
    ...
    参数:
        ...
        uid <- 认证用户ID。
    '''
    def __init__(self, dbcon:PgDB, schema, table):
        self.con = dbcon
        self.schema = schema
        self.table = table
    ...
    def modify_many(self,sql,*args,**kwargs):
        '''执行DML SQL 语句：
        首先 需要先执行 self.begin()
        然后 需要提交事务 self.commit()
        最后 能得到
        影响行数 <- [self.rowcount]
        '''
        self.con.modify(sql,*args,**kwargs)
    ...
    def insert_one(self,sql,*args,**kwargs):
        ...
        插入ID <- [self.lastid]
        '''
        self.con.insert(sql,*args,**kwargs)


    # ################# 查询补充 ################## 
    def search(self, **where) -> pl.DataFrame:
        '''
         参数：
            where {column:value ...}
        '''
        ...
        return self.bysql(sql, where=params)
    


    def byid(self,id)->dict:
        '''查询id数据，返回dict'''
        for ss in self.search(where={"id":id}).iter_rows(named=True):
            return ss
    ...
    def bysql_for_total_detail(self,sqltext:str,**where):
        ...
        data = {}
        df = self.bysql(sqltext,where=params)
        if not df.is_empty():
